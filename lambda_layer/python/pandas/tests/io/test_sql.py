...
def create_and_load_postgres_datetz(conn):
    ...
    metadata = MetaData()
    datetz = Table("datetz", metadata, Column(
        "DateColWithTz", DateTime(timezone=True)))
    datetz_data = [
        {
...
def test_dataframe_to_sql_arrow_dtypes(conn, request):
    ...
    with tm.assert_produces_warning(exp_warning, match=msg, check_stacklevel=False):
        df.to_sql(name="test_arrow", con=conn,
...
def test_insertion_method_on_conflict_update(conn, request):
    ...
    df.to_sql(name="test_insert_conflict", con=conn,
...
def test_api_date_parsing(conn, request):
    ...
    df = sql.read_sql_query("SELECT * FROM types",
                            conn, parse_dates=["DateCol"])
        ...
        expected = Series(
            [
                pd.DateOffset(months=0, days=0,
                              microseconds=1000000, nanoseconds=0),
                pd.DateOffset(months=0, days=0,
                              microseconds=3000000, nanoseconds=0),
            ],
            name="foo",
        )
    else:
...
def test_api_to_sql_index_label_multiindex(conn, request):
    ...
    result = sql.to_sql(temp_frame, "test_index_label",
...
def test_api_get_schema_dtypes(conn, request):
    ...
    create_sql = sql.get_schema(
        float_frame, "test", con=conn, dtype={"b": dtype})
    assert "CREATE" in create_sql
...
def test_database_uri_string(conn, request, test_frame1):
    ...
    with tm.ensure_clean() as name:
        db_uri = "sqlite:///" + name
        table = "iris"
        test_frame1.to_sql(name=table, con=db_uri,
                           if_exists="replace", index=False)
...
def test_sql_open_close(test_frame3):
    ...
    with tm.ensure_clean() as name:
        with closing(sqlite3.connect(name)) as conn:
            assert sql.to_sql(test_frame3, "test_frame3_legacy",
                              conn, index=False) == 4
...
def test_naive_datetimeindex_roundtrip(conn, request):
    ...
    assert expected.to_sql(name="foo_table", con=conn,
...
def test_date_parsing(conn, request):
    ...
    df = sql.read_sql_table("types", conn, parse_dates={
                            "DateCol": "%Y-%m-%d %H:%M:%S"})
    assert issubclass(df.DateCol.dtype.type, np.datetime64)
...
def test_dtype(conn, request):
    ...
    # GH9083
    assert df.to_sql(name="dtype_test3", con=conn,
                     dtype={"B": String(10)}) == 2
    meta.reflect(bind=conn)
...
def test_row_object_is_named_tuple(sqlite_engine):
    ...
    with Session() as session:
        ...
        assert (
            df.to_sql(name="test_frame", con=conn,
...
def test_psycopg2_schema_support(postgresql_psycopg2_engine):
    ...
    assert (
        df.to_sql(name="test_schema_other", con=conn,
...
def test_xsqlite_basic(sqlite_buildin):
    ...
    assert sql.to_sql(frame, name="test_table",
                      con=sqlite_buildin, index=False) == 10
    ...
    assert sql.to_sql(frame2, name="test_table2",
                      con=sqlite_buildin, index=False) == 10
    result = sql.read_sql("select * from test_table2",
                          sqlite_buildin, index_col="Idx")
    expected = frame.copy()
