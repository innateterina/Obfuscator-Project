...
class TestDataFramePlots:
    ...
    def test_andrews_curves_cmap(self, request, df):
        ...
        _check_colors(ax.get_lines()[:10],
                      linecolors=cmaps, mapping=df["Name"][:10])
    ...
    def test_parallel_coordinates_colors(self, iris, color):
        ...
        _check_colors(ax.get_lines()[:10],
                      linecolors=color, mapping=df["Name"][:10])
    ...
    def test_radviz_color(self, iris, color):
        ...
        df = iris
        ax = _check_plot_works(
            radviz, frame=df, class_column="Name", color=color)
    ...
    def test_dictionary_color(self, kind):
        ...
        df1 = DataFrame(np.random.default_rng(
            2).random((2, 2)), columns=data_files)
