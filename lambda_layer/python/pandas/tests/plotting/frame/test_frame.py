...
class TestDataFramePlots:
    ...
    def test_plot_int_columns(self):
        df = DataFrame(np.random.default_rng(
            2).standard_normal((100, 4))).cumsum()
    ...
    def test_errorbar_scatter_color(self):
        def _check_errorbar_color(containers, expected, has_err="has_xerr"):
            lines = []
            errs = next(c.lines for c in ax.containers if getattr(
                c, has_err, False))
            for el in errs:
                if is_list_like(el):
                    lines.extend(el)
            ...
            _check_colors(err_lines, linecolors=np.array(
